...
namespace ServerCore
{
    ...
    public abstract class Session
    {
        ...
        {
            _recvBuffer.Clean();
            ArraySegment<byte> segment = _recvBuffer.WriteSegment; // 수신 가능한 배열 추출
        ...
        void OnRecvCompleted(object sender, SocketAsyncEventArgs args)
        {
            if (args.BytesTransferred > 0 && args.SocketError == SocketError.Success)
            {
                try
                {
                    ...
                    int proccessLen = OnRecv(_recvBuffer.ReadSegment);
                    if(proccessLen < 0 || _recvBuffer.DataSize < proccessLen)
                    {
                        Disconnect();
                        return;
                    }
                    ...
                    // Read 커서 이동
                    if(_recvBuffer.OnRead(proccessLen) == false)
                    {
                        Disconnect();
                        return;
                    }
